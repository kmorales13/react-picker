...
export default class Picker extends React.PureComponent {
  ...
  componentDidMount() {
    ...
    if (op) {
      opHeight = this.state.optionHeight = ReactDOM.findDOMNode(
        op
      ).clientHeight;
    }
    ...
    values.forEach((v, idx) => {
      if (values[idx] !== preValues[idx] || opArr[idx] !== preOpArr[idx]) {
        ...
        for (let oi = 0; oi < ops.length; oi++) {
          let opv =
            typeof ops[oi] === 'string' || typeof ops[oi] === 'number'
              ? ops[oi]
              : ops[oi].value;
  ...
  _onScroll(e) {
    let el = e.target,
      idx = parseInt(
        el.dataset ? el.dataset.id : el.getAttribute('data-id'),
        10
      ),
      opHeight = this.state.optionHeight,
    ...
    this.state._scrollTimer = window.setTimeout(() => {
      ...
      let op = ReactDOM.findDOMNode(this.refs[opname]).getAttribute(
        'data-value'
      );
      if (!op) return;
      op = JSON.parse(op);
      ...
      <div className={['picker', this.props.className].join(' ')}>
        ...
        <div
          className={[
            ...
            this.state.open ? 'show' : undefined
          ].join(' ')}
        >
